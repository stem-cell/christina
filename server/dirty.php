...
// By the way, that SfYaml library above is slow as fuck. I'm telling you, that
// thing is a bottleneck for performance.
...
if (isset($_GET['apc'])) { echo 'APC installed and enabled: '.($isThatThingInstalled ? 'yes' : 'no'); exit; }
// For completeness' sake, there's also memcache/d, even thought we currently don't have code paths that use them:
if (isset($_GET['memcache'])) { echo 'Memcache installed: '.(class_exists('Memcache') ? 'yes' : 'no'); exit; }
if (isset($_GET['memcached'])) { echo 'Memcached installed: '.(class_exists('Memcached') ? 'yes' : 'no'); exit; }
